...
export default function FilePreview({ file }: FilePreviewProps) {
    ...
    const imgSrc: string | undefined = file.type.startsWith("image/")
        ? URL.createObjectURL(file)
        : undefined;
    const pdfSrc: string | undefined = file.type.startsWith("application/pdf")
        ? URL.createObjectURL(file)
        : undefined;
    ...
    useEffect(() => {
        if (
            isPreviewVisible &&
            !text &&
            (file.type.startsWith("text/") || file.type.startsWith("application/json"))
        ) {
            loadText();
        } else {
            setOffset(0);
